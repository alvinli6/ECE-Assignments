...
void pipe_cycle_IF(Pipeline *p){
  ...
  for(ii=0; ii<PIPE_WIDTH; ii++){
    ...
    if(!p->pipe_latch[IF_LATCH][ii].stall && !p->fetch_cbr_stall){
			...
			if(BPRED_POLICY>0 && fetch_op.tr_entry.op_type == OP_CBR){
...
void pipe_check_bpred(Pipeline *p, Pipeline_Latch *fetch_op){
  ...
  // stall fetch using the flag p->fetch_cbr_stall
	...
	bool takenOrNot = p->b_pred->GetPrediction(BPRED_POLICY);
	p->b_pred->UpdatePredictor(fetch_op->tr_entry.inst_addr, fetch_op->tr_entry.br_dir, takenOrNot);

	if (takenOrNot != fetch_op->tr_entry.br_dir) {
