...
void analyze_trace_record(Trace_Rec *t){
    ...
    //task 1, dynamic count for each type of instruction
    //task 2, cycles incremented depending on instruction type
    ...
    //task 3, count unique inst_addr's using global set 
    if (instAddrSet.find(t->inst_addr) == instAddrSet.end()) {
    	instAddrSet.insert(t->inst_addr);
    	stat_unique_pc++;
    }
}
