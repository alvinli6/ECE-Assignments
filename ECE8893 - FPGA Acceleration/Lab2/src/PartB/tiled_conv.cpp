...
#pragma HLS INTERFACE s_axilite register	port=return
    ...
    for(int ti = 0; ti < N_TILE_ROWS; ti++)
    {
        for(int tj = 0; tj < N_TILE_COLS; tj++)
        {
            ...
            for (int b = 0; b < 4; b++) {
            	for (int w = 0; w < 8; w++) {
            		load_input_tile_block_from_DRAM(conv_in_buf, input_feature_map, ti, tj, w);
            		load_layer_params_from_DRAM(conv_wt_buf, conv_bias_buf, layer_weights, layer_bias, b, w);
            		conv_3x3(conv_out_buf, conv_in_buf, conv_wt_buf);
            		save_partial_output_tile_block(partial_out_fm_buf, conv_out_buf, conv_bias_buf, w);
            	}
